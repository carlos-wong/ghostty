...
/// The `new-tab` will use native platform IPC to open up a new tab in a
/// running instance of Ghostty.
///
/// If the `--class` flag is not set, the `new-tab` command will try and
/// connect to a running instance of Ghostty based on what optimizations the
/// Ghostty CLI was compiled with. Otherwise the `new-tab` command will try
...
/// in the new tab rather than the default. If `-e` is not specified, Ghostty
...
/// it automatically.
///
/// On GTK, D-Bus activation must be properly configured. See the Ghostty
/// website for information on properly configuring D-Bus activation.
///
/// Flags:
///
